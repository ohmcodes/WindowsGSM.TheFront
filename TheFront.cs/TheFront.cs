...
namespace WindowsGSM.Plugins
{
    public class TheFront : SteamCMDAgent
    {
        ...
        //public string ConfigServerName = GenerateRandom12DigitNumber(random);
        //string ConfigServerName = RandomNumberGenerator.Generate12DigitRandomNumber();
        ...
        public string Defaultmap = "TheFront"; // Original (MapName)
        ...
        public string Additional = $"-EnableParallelCharacterMovementTickFunction -EnableParallelCharacterTickFunction -UseDynamicPhysicsScene -fullcrashdumpalways -Game.PhysicsVehicle=false -ansimalloc -Game.MaxFrameRate=35 -MaxQueueSize=50 -QueueValidTime=120";
        ...
        public async Task<Process> Start()
        {
            ...
            string param =  $"ProjectWar_Start?DedicatedServer?MaxPlayers={_serverData.ServerMaxPlayer}?udrs=steam -server -game -log";

            param += $" -ConfigServerName=\"{ConfigServerName}\"";
            param += $" -OutIPAddress={localIpAddress}";
            param += $" -UserDir=\"{ServerPath.GetServersServerFiles(_serverData.ServerID)}\"";
            param += $" -ServerName=\"{_serverData.ServerName}\"";
            param += $" -Port=\"{_serverData.ServerPort}\"";
            param += $" -QueryPort=\"{_serverData.ServerQueryPort}\"";
            param += $" -BeaconPort=\"27049\"";
            param += $" -ShutDownServicePort=\"27050\" ";
